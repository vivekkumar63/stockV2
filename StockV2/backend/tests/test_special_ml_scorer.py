...
def test_predict_returns_none_when_no_model():
    ...
    result = scorer.predict({
        "strategy_id": 999, "entry_month": 6, "entry_dow": 2,
...
def test_train_returns_dict_with_samples_zero_when_insufficient():
    ...
    scorer = SpecialMLScorer()
    result = scorer.train(db, special_strategy_id=1)
    assert isinstance(result, dict)
    assert result["samples"] == 0


def test_train_returns_dict_with_samples_zero_when_single_class():
    """All trades profitable → only one class → skipped."""
    db = MagicMock()
    fake_rows = [
        (1, date(2023, i % 12 + 1, 1), "BULL", 500.0, 0.65, 1.8, 4.2, None, None, None, None)
        for i in range(60)
    ]
